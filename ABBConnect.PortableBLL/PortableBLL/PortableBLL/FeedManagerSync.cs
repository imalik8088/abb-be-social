...
namespace PortableBLL
{
    ...
    {
           SyncAccesser acceser;

           public FeedManagerSync()
            {
                acceser = new SyncAccesser();
            }

            public List<Feed> LoadLatestXFeeds(int feedNum)
            {
                List<GetLatestXFeeds_Result> list =  acceser.GetLatestXFeeds(feedNum);
            ...
            {
                List<GetLatestXFeeds_Result> list = acceser.GetLatestXFeedsFromId(numberOfFeeds, startingId);
